...
while True:
    ...
    if result.is_valid():             #valid result check
        ...
        urle = 'http://ebsr12.transsyssolutions.com/webservices/rest/XXJET_PROFILE_PKG/dashboard/'
        payload = {
                   'InputParameters':
                      {
                      'P_PERSONID': 30981
        ...
        headers = {'Content-Type': 'application/json', 'authorization': 'Basic b3BlcmF0aW9uczp3ZWxjb21l'}
        r = requests.post(urle, data=json.dumps(payload), headers=headers)
        print(r.content)
